export_all <- function(outfile, input){
  wb <- createWorkbook(type="xlsx")
  
  for(val_desagrega in )
}
wb <- createWorkbook(type="xlsx")
...
# Diseno de hoja
sheet <- createSheet(wb, sheetName = "probit0x")
setZoom(sheet, numerator = 85, denominator = 100)
posicion_tablas <- c(3, 2, 1)

# Poner data
indicTable <- tabla_normalizada %>%
  subset(desag == "NACIONAL") %>%
  left_join(
    tabla_normalizada %>%
      subset(desag == "AMAZONAS"),
    by = "nombre") %>%
...
xlsx.addTitle(sheet,
              rowI = 1, colI = 2,
              title = "wako", titleStyle = TITLE_STYLE)
